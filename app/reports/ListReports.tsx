...
const ListReports = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (user?.roles === "system_user") {
        res = await activityLogApi.findAllReportByUserId(user?.id as string);
      }
      if (user?.roles === "system_fixer") {
        res = await activityLogApi.findAllReportByFixerId(user?.id as string);
      }
      setBills(res || []);
    } catch (error) {
    ...
    <View className="bg-white rounded-xl p-4 mb-3 shadow-sm">
      <View className="flex-row justify-between items-center mb-2">
        <Text className="text-base font-semibold text-gray-800">
          Mã báo cáo: {item.id?.slice(0, 13) || "N/A"}
        </Text>
  ...
  if (!bills.length) {
    return (
      <View className="flex-1 bg-gray-50 justify-center items-center">
        <Text className="text-gray-500">Bạn chưa tạo báo cáo nào</Text>
      </View>
    );
  }
